...
from utils.telegram_send_file import send_to_telegram, remove_temp_file
...
VIDEO_RECORD_TIME      = VIDEO["RECORD"]["DEFAULT_RECORD_TIME"]
...
def record_a_video(record_time_sec, video_path=None):
    ...
    video_full_path = EVENT_VIDEO_FILE_PATH if video_path is None else video_path
    print("Captured video saved on: {}".format(video_full_path))

    # Create a video write before entering the loop
    video_writer_out = cv2.VideoWriter(
        video_full_path, VIDEO_CODEC, VIDEO_RECORD_FPS, (int(vcap.get(3)), int(vcap.get(4)))
    )
    ...
    video_duration(video_full_path)
